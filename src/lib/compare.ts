...
export interface Row {
  ...
  b: AnnotatedValue[];
  note?: string;
  /** Collapsed sitelink rows only (collapseRepeatedRows): every wiki sharing these values. */
  sites?: string[];
}
...
/**
 * Collapses label and sitelink rows that carry exactly the same values (and
 * status/notes) into one row listing every language or wiki, e.g. seven
 * "label (de)" … "label (nl)" rows all reading "Teen Angels - La Despedida" vs
 * "Teen Angels: La Despedida" become a single "label (de, en, es, fi, fr, ga,
 * nl)" row, and enwiki/eswiki/mkwiki sitelinks all titled "Crush 'Em" become one
 * "enwiki, eswiki, mkwiki" row carrying the wikis in `sites`. Blocker rows (two
 * different pages on one wiki) are never merged, so each clash keeps its own
 * row and note; redirect sitelinks differ in their values' `redirect` flag and
 * so only group with other redirects. The merged row takes the first row's
 * place; rows that don't share their values are left as-is.
 */
export function collapseRepeatedRows(rows: Row[]): Row[] {
  const prefix = (r: Row): string | undefined =>
    r.blocker ? undefined : ["label:", "sitelink:"].find((p) => r.key.startsWith(p));
  const signature = (r: Row): string => JSON.stringify([prefix(r), r.status, r.note, r.a, r.b]);
  const namesBySig = new Map<string, string[]>();
  for (const r of rows) {
    const p = prefix(r);
    if (!p) continue;
    const sig = signature(r);
    const names = namesBySig.get(sig) ?? [];
    names.push(r.key.slice(p.length));
    namesBySig.set(sig, names);
  }
  const emitted = new Set<string>();
  const out: Row[] = [];
  for (const r of rows) {
    const p = prefix(r);
    const names = p ? namesBySig.get(signature(r)) : undefined;
    if (!p || !names || names.length < 2) {
      out.push(r);
      continue;
    }
    const sig = signature(r);
    ...
    out.push(
      p === "label:"
        ? { ...r, key: `label:${names.join(",")}`, label: `label (${names.join(", ")})` }
        : { ...r, key: `sitelink:${names.join(",")}`, label: names.join(", "), sites: names },
    );
  }
  return out;
}
